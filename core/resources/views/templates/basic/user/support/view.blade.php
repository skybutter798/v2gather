                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('ticket.reply', $myTicket->id) }}" enctype="multipart/form-data">
                            ...
                            <div class="text-end">
                                <a href="javascript:void(0)" class="btn {{ auth()->user() ? 'btn--primary' : ' btn--base' }} btn-sm addFile">
                            ...
                            <div class="form-group">
                                ...
                                <input type="file" name="attachments[]" class="form-control form--control form-control-lg {{ !auth()->user() ? 'padding-top' : '' }}" />
                            ...
                            <button type="submit" class="btn {{ auth()->user() ? 'btn--primary' : 'btn--base' }}  w-100 h-45"> <i class="fa fa-reply"></i>
                                @lang('Reply')</button>
                        </form>
                    </div>
                </div>
                                        ...
                                        @if ($message->attachments->count() > 0)
                                            <div class="mt-2">
                                                @foreach ($message->attachments as $k => $image)
                                                    <a href="{{ route('ticket.download', encrypt($image->id)) }}" class="me-3 ticketAtt">
...
@push('breadcrumb-plugins')
    <x-back route="{{ route('ticket.index') }}"></x-back>
@endpush
            ...
            $('.addFile').on('click', function() {
                ...
                $("#fileUploadsContainer").append(`
                    <div class="input-group my-3">
                        <input type="file" name="attachments[]" class="form-control form--control form-control-lg {{ !auth()->user() ? 'padding-top' : '' }}" required />
